...
def build_imagefolder_dataset(opt, log, train, transform=None):
    fn = opt.dataset_dir / ('train' if train else 'val')
    ...
    dataset = datasets.ImageFolder(fn, transform=transform)
    log.info(f"[Dataset] Built ImageFolder dataset {fn=}, size={len(dataset)}!")
    return dataset

def build_dataset(opt, log, train, transform=None):
    if getattr(opt, "dataset_format", "lmdb") == "imagefolder":
        return build_imagefolder_dataset(opt, log, train, transform=transform)
    return build_lmdb_dataset(opt, log, train, transform=transform)

def readlines(fn):
    file = open(fn, "r").readlines()
    return [line.strip('\n\r') for line in file]

def _remap_val10k_filenames(opt, log, filenames):
    remapped = []
    for fn in filenames:
        if os.path.exists(fn):
            ...
            continue
        if "/val/" in fn:
            rel_path = fn.split("/val/")[1]
            remapped.append(str(opt.dataset_dir / "val" / rel_path))
            continue
        remapped.append(fn)
        log.warning(f"[Dataset] Could not remap val10k path: {fn}")
    return remapped

def build_dataset_val10k(opt, log, transform=None):
    ...
    dataset = build_dataset(opt, log, train=False, transform=transform)
    if getattr(opt, "dataset_format", "lmdb") == "imagefolder":
        fn_10k = _remap_val10k_filenames(opt, log, fn_10k)
...
class InpaintingVal10kSubset(Dataset):
    def __init__(self, opt, log, mask):
        ...
        self.mask_type = mask
        self.dataset = build_dataset_val10k(opt, log)
