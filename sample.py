...
def build_val_dataset(opt, log, corrupt_type):
    if "sr4x" in corrupt_type:
        val_dataset = imagenet.build_dataset(opt, log, train=False)  # full 50k val
        ...
        val_dataset = imagenet.build_dataset_val10k(opt, log)
        val_dataset = MixtureCorruptDatasetVal(opt, val_dataset)  # subset 10k val + mixture
    else:
        val_dataset = imagenet.build_dataset_val10k(opt, log)  # subset 10k val
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset-format", type=str, default="lmdb", choices=["lmdb", "imagefolder"],
